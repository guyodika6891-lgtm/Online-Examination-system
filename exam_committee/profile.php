...
$stmt = $pdo->prepare("
    SELECT 
        (SELECT COUNT(*) FROM exam_schedules WHERE created_by = ?) as exams_created,
        (SELECT COUNT(*) FROM questions WHERE approved_by = ?) as questions_approved,
        (SELECT COUNT(*) FROM exam_schedules WHERE status = 'ongoing') as active_exams
");
...
<html>
<head>
    <title>Settings - Exam Committee</title>
    ...
    <style>
        ...
        .profile-header { background: linear-gradient(135deg, #667eea, #764ba2); padding: 30px; text-align: center; color: white; }
        ...
        .profile-stats { padding: 20px; background: #f8f9fa; display: flex; justify-content: space-around; text-align: center; }
        ...
        .form-group input.success { border-color: #28a745; }
        .btn-primary { background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 12px 25px; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; transition: all 0.3s; }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(102,126,234,0.3); }
        .btn-secondary { background: #6c757d; color: white; padding: 10px 20px; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; }
        ...
        .validation-message { font-size: 11px; margin-top: 5px; }
...
<body>
                ...
                <div class="settings-grid">
                    ...
                    <div class="profile-card">
                        <div class="profile-header">
                            ...
                            <div class="profile-role">Exam Committee Member</div>
                        </div>
                        <div class="profile-stats">
                            <div class="stat-item">
                                <div class="stat-value"><?php echo $stats['exams_created']; ?></div>
                                <div class="stat-label">Exams Created</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value"><?php echo $stats['questions_approved']; ?></div>
                                <div class="stat-label">Questions Approved</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value"><?php echo $stats['active_exams']; ?></div>
                    ...
                    <div>
                        ...
                        <div class="settings-card">
                            ...
                            <form method="POST" onsubmit="return validatePassword()">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="password" name="new_password" id="new_password" required onkeyup="validateNewPassword()">
                                    <div class="validation-message" id="password_strength"></div>
                                    <div class="info-text">Minimum 6 characters</div>
                                </div>
                                
                                <div class="form-group">
                                    <label>Confirm New Password</label>
                                    <input type="password" name="confirm_password" id="confirm_password" required onkeyup="validateConfirmPassword()">
                                    <div class="validation-message" id="confirm_validation"></div>
                                </div>
                        ...
                        <div class="settings-card">
                            ...
                            <div style="display: grid; gap: 10px;">
                                <div><strong>Account Created:</strong> <?php echo date('F j, Y', strtotime($user['created_at'])); ?></div>
                                <div><strong>Last Login:</strong> <?php echo $user['last_login'] ? date('F j, Y g:i A', strtotime($user['last_login'])) : 'Never'; ?></div>
    ...
    <script>
        ...
        function validateNewPassword() {
            ...
            const validationDiv = document.getElementById('password_strength');
            
            if (password.length > 0) {
                if (password.length >= 6) {
                    validationDiv.innerHTML = '<i class="fas fa-check-circle"></i> Password strength: Good';
                    validationDiv.className = 'validation-message success';
                } else {
        ...
        function validateConfirmPassword() {
            ...
            const validationDiv = document.getElementById('confirm_validation');
