...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['schedule_exam'])) {
    ...
    if($duration_minutes < 15) $errors[] = "Duration must be at least 15 minutes";
    if($total_questions < 1) $errors[] = "At least 1 question is required";
    
    if(empty($errors)) {
        try {
            // Get total marks for the selected questions
            ...
            if($total_marks == 0) {
                $total_marks = $total_questions; // Default if no marks found
            }
